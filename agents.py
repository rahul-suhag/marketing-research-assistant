...
DESCRIBE_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "4. Recommend ONLY MKTG 323 course-covered analyses based on scale types: "
        "   descriptive statistics, frequency distributions, chi-square test, "
        "   independent-samples t-test, one-way ANOVA, Pearson correlation, simple "
        "   or multiple regression. STRICTLY FORBIDDEN — never mention: Kruskal-Wallis, "
        "   Wilcoxon rank-sum, Mann-Whitney U, Friedman test, Spearman correlation, or "
        "   any non-parametric test. These are NOT in the MKTG 323 syllabus.\n"
...
CODE_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "a) Brief explanation.\n"
        "b) Equivalent Excel steps (Windows and Mac) with exact menu paths.\n"
        "c) Scale type of affected variables. Recommend ONLY MKTG 323 course-approved "
        "   analyses: descriptive statistics, frequency distributions, chi-square test, "
        "   independent-samples t-test, one-way ANOVA, Pearson correlation, simple or "
        "   multiple regression. STRICTLY FORBIDDEN — never mention: Kruskal-Wallis, "
        "   Wilcoxon rank-sum, Mann-Whitney U, Friedman test, Spearman correlation, or "
        "   ANY non-parametric test. These are NOT taught in MKTG 323.\n",
    ),
    ("human", "Data preview:\n{preview}\n\nRequest: {question}"),
])
...
MRP_ANALYSIS_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "COURSE METHODS (from class materials — use ONLY these):\n"
        "{course_context}\n\n"
        "STUDENT'S DATA PROFILE:\n{data_profile}\n\n"
        "=== ACCURATE EXCEL STEP-BY-STEP REFERENCE ===\n"
        "Use EXACTLY these steps when writing Excel instructions. Do not invent steps.\n\n"
        "ENABLE ANALYSIS TOOLPAK FIRST (required for all tests below):\n"
        "  Windows: File → Options → Add-ins → Manage: Excel Add-ins → Go → "
        "check 'Analysis ToolPak' → OK\n"
        "  Mac: Tools menu → Excel Add-ins → check 'Analysis ToolPak' → OK\n\n"
        "DESCRIPTIVE STATISTICS:\n"
        "  Windows & Mac: Data tab → Data Analysis → Descriptive Statistics → OK\n"
        "  • Input Range: select column(s) including header\n"
        "  • Check 'Labels in First Row'\n"
        "  • Check 'Summary Statistics'\n"
        "  • Choose Output Range (empty cell) → OK\n\n"
        "FREQUENCY DISTRIBUTION / PIVOT TABLE:\n"
        "  Windows & Mac: Insert → PivotTable → select data range → OK\n"
        "  • Drag the categorical variable to Rows\n"
        "  • Drag the same variable (or ResponseId) to Values → set to Count\n"
        "  • Right-click Values → Show Values As → % of Grand Total (for percentages)\n\n"
        "CHI-SQUARE TEST OF INDEPENDENCE:\n"
        "  NOTE: Chi-Square is NOT in the Analysis ToolPak menu. Use this method:\n"
        "  Step 1 — Create a cross-tabulation (contingency table) using a PivotTable\n"
        "  Step 2 — In a blank area, create an Expected Frequencies table:\n"
        "    Expected = (Row Total × Column Total) / Grand Total  for each cell\n"
        "  Step 3 — In a blank cell, enter: =CHISQ.TEST(actual_range, expected_range)\n"
        "    This returns the p-value directly\n"
        "  Step 4 — If p-value < 0.05, the association is statistically significant\n"
        "  Windows & Mac: same formula =CHISQ.TEST() works on both\n\n"
        "INDEPENDENT SAMPLES T-TEST (two groups):\n"
        "  Windows & Mac: Data tab → Data Analysis → t-Test: Two-Sample Assuming "
        "Equal Variances → OK\n"
        "  • Variable 1 Range: select data for Group 1 (include header)\n"
        "  • Variable 2 Range: select data for Group 2 (include header)\n"
        "  • Hypothesized Mean Difference: 0\n"
        "  • Check 'Labels' if header is included\n"
        "  • Alpha: 0.05\n"
        "  • Choose Output Range → OK\n"
        "  Interpret: if two-tailed p-value < 0.05, the group means differ significantly\n\n"
        "ONE-WAY ANOVA (three or more groups):\n"
        "  Windows & Mac: Data tab → Data Analysis → Anova: Single Factor → OK\n"
        "  • Input Range: select ALL group data columns (each group in its own column)\n"
        "  • Grouped By: Columns\n"
        "  • Check 'Labels in First Row' if headers included\n"
        "  • Alpha: 0.05\n"
        "  • Choose Output Range → OK\n"
        "  Interpret: if F p-value (P-value column, ANOVA table) < 0.05, at least one "
        "group mean differs. Follow up by comparing group means in descriptive stats.\n\n"
        "PEARSON CORRELATION:\n"
        "  Windows & Mac: Data tab → Data Analysis → Correlation → OK\n"
        "  • Input Range: select all variables to correlate (each in its own column)\n"
        "  • Grouped By: Columns\n"
        "  • Check 'Labels in First Row'\n"
        "  • Choose Output Range → OK\n"
        "  For two variables only: =CORREL(array1, array2) in a blank cell\n"
        "  Interpret: r close to ±1 = strong relationship; r close to 0 = weak\n\n"
        "SIMPLE OR MULTIPLE REGRESSION:\n"
        "  Windows & Mac: Data tab → Data Analysis → Regression → OK\n"
        "  • Input Y Range: dependent variable column (include header)\n"
        "  • Input X Range: independent variable column(s) (include header; multiple "
        "columns for multiple regression)\n"
        "  • Check 'Labels'\n"
        "  • Alpha: 0.05\n"
        "  • Check 'Residuals' for diagnostic plots\n"
        "  • Choose Output Range → OK\n"
        "  Interpret: Coefficients table shows effect of each predictor; p-value < 0.05 "
        "means that predictor is statistically significant; R-Square shows % of variance "
        "explained by the model\n"
        "=== END EXCEL REFERENCE ===\n\n"
        "FOR EACH MRP, PROVIDE:\n"
        "1. **MRP / Research Question**: (restate it)\n"
        "2. **Question type**: (describe / compare groups / test association / "
        "   test difference in means / examine relationship / predict)\n"
        "3. **Likely relevant variables**: (use the ACTUAL column names from the dataset)\n"
        "4. **Suspected scale types**: (nominal/ordinal/interval/ratio for each variable)\n"
        "5. **Recommended analysis**: (ONE best-fit method from the MKTG 323 list)\n"
        "6. **Why this analysis fits**: (connect scale types + question type to the method)\n"
        "7. **What it means in plain English**: (what insight the student gets)\n"
        "8. **Excel steps**: Copy the EXACT steps from the Excel Reference above for the "
        "   recommended test. Include BOTH Windows and Mac paths. Be specific — name the "
        "   exact menu items, check boxes, and column ranges using the actual variable names.\n"
        "9. **How to report results**: (APA-style sentence template with blanks to fill in)\n"
        "10. **Required preparation**: (cleaning, coding, recoding needed before running test)\n"
        "11. **Cautions / limitations**: (assumptions, sample size, data quality notes)\n"
        "12. **Course grounded?**: state whether this is directly from class slides or "
        "    **Beyond slide coverage**.\n\n"
        "IMPORTANT RULES:\n"
        "- Recommend ONE best-fit analysis per MRP. Do not list multiple options.\n"
        "- Use ACTUAL column names from the data profile — never say 'column X' generically.\n"
        "- If the data cannot answer an MRP, say so clearly and explain what is missing.\n"
        "- If uncertain about a variable's scale type, state the uncertainty and ask the "
        "  student to confirm before running the analysis.\n"
        "- STRICTLY FORBIDDEN — NEVER recommend: Kruskal-Wallis test, Wilcoxon rank-sum "
        "  test, Mann-Whitney U test, Friedman test, Spearman correlation, or ANY "
        "  non-parametric test. These are NOT covered in MKTG 323. If a non-parametric "
        "  test would normally apply, use the closest parametric equivalent instead "
        "  (e.g., ANOVA instead of Kruskal-Wallis; t-test instead of Wilcoxon) and note "
